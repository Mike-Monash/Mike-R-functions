...
qt <- function(input_data){   # simple function for listing category variables - wrapper for table function
  ...
  if ((input_data %notin% colnames(df)) == TRUE ) {
    ...
    if(show_tables == TRUE){ return(return_list) }
  }
}
...
qxt <- function(input_data, output_data){
  ...
  if ((input_data %notin% colnames(df)) == TRUE) {
    print("Input column not found in default dataframe")
    input_i <- 0
       }

  if ((output_data %notin% colnames(df)) == TRUE) {
    print("Output column not found in default dataframe")
    output_i <- 0
         }

  if(input_i * output_i == 1 ) {

    s2 <- glue ("table_x <- table({default_data}$`{input_data}`, {default_data}$`{output_data}`, useNA = 'always')")
    eval(parse(text = s2))

  if(show_tables == TRUE){ return(table_x)  }
  }
}
...
mike_date_column <- function(column){

  col_type <- class(column)

  if(col_type %notin% c('POSIXlt','POSIXct')) {

    new_column <- lapply(column, mikedate) %>% unlist() %>% as.Date(origin="1970-01-01")
